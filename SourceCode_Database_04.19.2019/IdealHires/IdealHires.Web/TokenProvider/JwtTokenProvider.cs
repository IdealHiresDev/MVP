...
namespace IdealHires.Web.TokenProvider
{
    public class JwtTokenProvider
    {
        ...
        public bool GetTokenAsync(string username, string password, string userType, out string token) //, string clientId, string deviceId
        {
            using (var client = new HttpClient())
            {
                client.BaseAddress = new Uri(_tokenUri);
                ...
                else
                {
                    ...
                    JObject obj = JObject.Parse(errorResult);
                    token = obj["error_description"].ToString();
                    return false;
                }
            }
        }
        ...
        public ClaimsIdentity CreateIdentity(bool isAuthenticated, string userName, string token)
        {
            ...
            var jwtIdentity = new ClaimsIdentity(new JwtIdentity(
                isAuthenticated, userName, DefaultAuthenticationTypes.ApplicationCookie, userType
                    ));
        ...
        public class JwtIdentity : IIdentity
        {
            ...
            public JwtIdentity(bool isAuthenticated, string name, string authenticationType,string type)
            {
                _isAuthenticated = isAuthenticated;
                _name = name;
                _authenticationType = authenticationType;
