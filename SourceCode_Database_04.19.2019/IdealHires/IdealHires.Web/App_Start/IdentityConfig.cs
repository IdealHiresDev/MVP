...
{    
    ...
    public class ApplicationSignInManager : SignInManager<UserDTO, int>
    {
        ...
        //authenticate from external authorization server
        //and create a claim
        public SignInStatus LoginInAsync(LoginDTO login, bool isPersistent, bool shouldLockout, out string message)
        {
            string token = string.Empty;
            string uri = ConfigurationManager.AppSettings["apiUrl"] + "/token";
            var jwtProvider = TokenProvider.JwtTokenProvider.Create(uri);
            bool status = jwtProvider.GetTokenAsync(login.Email, login.Password, login.UserType, out token); //, clientId, Environment.MachineName
            if (!status)
            {
                message = token;
                return SignInStatus.Failure;
            ...
            {
                //create an Identity Claim
                ClaimsIdentity claims = jwtProvider.CreateIdentity(true, login.Email, token);
